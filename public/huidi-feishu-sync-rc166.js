...
const online=Boolean(window.HUIDI_COMMUNITY_ONLINE?.enabled);
...
async function api(path,{method='GET',body}={}){
  ...
  if(!res.ok||data?.ok===false)throw new Error(data?.message||(typeof data?.detail==='string'?data.detail:'')||data?.error||`请求未完成（${res.status}）`);
  return data;
}
...
function openConfig(status={}){const dialog=$('#appDialog');if(!dialog)return;dialog.innerHTML=`<div class="modal"><div class="modal-head"><div><h3>飞书连接与数据权限</h3><p>${online?'凭证加密保存在当前工作区服务器；其他公司不能读取。保存后先测试应用，再读取自己的表格。':'凭证只保存在软件目录私有配置文件中，不写入浏览器备份。'}</p></div><button class="close" type="button" data-feishu-close>×</button></div><div class="form-grid"><div class="field"><label>App ID</label><input class="input" id="feishuAppId" autocomplete="off" value="${esc(status.app_id||'')}" placeholder="cli_xxxxxxxxxx"></div><div class="field"><label>App Secret</label><input class="input" id="feishuAppSecret" type="password" autocomplete="new-password" placeholder="${status.configured?'留空表示保持现有 Secret':'填写自建应用 App Secret'}"></div><div class="field"><label>飞书企业域名（可选）</label><input class="input" id="feishuTenantDomain" value="${esc(status.tenant_domain||'')}" placeholder="example.feishu.cn"></div><div class="field"><label>目标文件夹 Token（可选）</label><input class="input" id="feishuFolderToken" value="${esc(status.folder_token||'')}" placeholder="fldcn...；留空则使用应用可创建的位置"></div><div class="field full"><label>已有文档 ID（可选）</label><input class="input" id="feishuDocumentId" value="${esc(status.document_id||'')}" placeholder="留空：首次同步自动创建；填写：后续快照追加到该文档"></div></div><div class="feishu-security-note"><b>需要的飞书权限</b><span>建议在飞书开放平台为你自己的自建应用开启所需的云空间、电子表格、多维表格以及文档读写权限。HUIDI 只能访问该应用实际获准访问的文件；如果指定文件夹，还需要把目标文件夹授权给该应用。</span></div><div class="modal-foot"><button class="btn" type="button" data-feishu-close>取消</button><button class="btn primary" type="button" id="feishuSaveConfig">保存配置</button></div></div>`;dialog.showModal();$('#feishuAppId')?.focus();const secret=$('#feishuAppSecret');const clearSecret=()=>{if(secret)secret.value='';};dialog.addEventListener('cancel',clearSecret,{once:true});dialog.addEventListener('close',()=>{clearSecret();dialog.removeEventListener('cancel',clearSecret);},{once:true});dialog.querySelectorAll('[data-feishu-close]').forEach(b=>b.onclick=()=>{clearSecret();dialog.close()});$('#feishuSaveConfig').onclick=async()=>{const body={app_id:clean($('#feishuAppId')?.value),app_secret:clean($('#feishuAppSecret')?.value),tenant_domain:clean($('#feishuTenantDomain')?.value),folder_token:clean($('#feishuFolderToken')?.value),document_id:clean($('#feishuDocumentId')?.value)};try{await api('/api/feishu/config',{method:'POST',body});clearSecret();dialog.close();toast(online?'飞书配置已加密保存，请测试连接。':'飞书本地配置已保存。');window.dispatchEvent(new Event('huidi:feishu-config-changed'));await refreshStatus()}catch(err){toast(`保存失败：${err.message}`,'error')}};}
...
window.addEventListener('click',e=>{const b=e.target.closest?.('[data-feishu-action]');if(!b)return;const a=b.dataset.feishuAction;if(!['config','test','sync','open'].includes(a))return;e.preventDefault();e.stopImmediatePropagation();if(a==='config')configure();if(a==='test')testConnection();if(a==='sync')sync();if(a==='open'&&b.dataset.url)window.open(b.dataset.url,'_blank','noopener,noreferrer')},true);
