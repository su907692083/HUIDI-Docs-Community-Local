/* HUIDI Docs Community Local RC16.14 — canonical issue locator/navigation. */
(()=>{
  'use strict';
  const VERSION='HUIDI-DOCS-COMMUNITY-LOCAL-1.2.0-RC16.14-ISSUE-NAVIGATOR';
  ...
  const formalObserver=new MutationObserver(()=>syncFormalDialog());
  if(document.body)formalObserver.observe(document.body,{childList:true,subtree:true});
  else document.addEventListener('DOMContentLoaded',()=>formalObserver.observe(document.body,{childList:true,subtree:true}),{once:true});
  if(!document.getElementById('huidi-issue-navigator-rc1614-style')){
    const style=document.createElement('style');style.id='huidi-issue-navigator-rc1614-style';
  ...
  document.documentElement.dataset.huidiIssueNavigator='rc16.14';
})();
