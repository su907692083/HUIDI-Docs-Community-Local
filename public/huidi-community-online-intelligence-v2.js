...
async function mountCustomerIntel(pane){pane.innerHTML='';pane.classList.add('fv2-mounted');await load('plain').catch(()=>null);const intel=await load('intelligence');await intel.mount(pane)}
...
async function mountWorldMap(pane){pane.innerHTML='';pane.classList.add('fv2-mounted');installMapRouting(pane);await load('plain').catch(()=>null);const intel=await load('intelligence');await intel.mount(pane);const country=await load('worldCountry');const map=await load('worldMap');await map.open();setTimeout(()=>country.enhance?.(),80)}
async function open(key){const pane=activateVisual(key);if(!pane)return;try{if(key==='customer-intel')await mountCustomerIntel(pane);if(key==='world-map')await mountWorldMap(pane)}catch(e){pane.innerHTML=`<div class="fv2-note warn">${String(e?.message||e||'市场情报暂时无法读取')}</div>`}}
function install(){const view=$('#view-online-intel'),bar=view?.querySelector(':scope > .fv2-tabs'),panes=view?.querySelector(':scope > .fv2-panes');if(!bar||!panes||bar.dataset.fv2IntelProjects==='1')return false;bar.dataset.fv2IntelProjects='1';const live=bar.querySelector('[data-fv2-tab="live"]');const world=makeTab('world-map','互动地图'),customer=makeTab('customer-intel','客户情报');if(live){live.insertAdjacentElement('afterend',customer);live.insertAdjacentElement('afterend',world)}else bar.append(world,customer);panes.append(makePane('world-map'),makePane('customer-intel'));bar.addEventListener('click',e=>{const b=e.target.closest('[data-fv2-tab="world-map"],[data-fv2-tab="customer-intel"]');if(b)setTimeout(()=>open(b.dataset.fv2Tab),0)});return true}
function boot(){if(!install())setTimeout(install,120);setTimeout(install,360)}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.HUIDICommunityOnlineIntelligenceV2=Object.freeze({version:'2.0.0',install,open,mountWorldMap,mountCustomerIntel});
})();
