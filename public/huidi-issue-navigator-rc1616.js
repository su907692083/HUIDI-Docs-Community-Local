/* HUIDI Docs Community Local RC16.18 — advisory issue locator/navigation, narrow-dialog observer. */
(()=>{
  'use strict';
  const VERSION='HUIDI-DOCS-COMMUNITY-LOCAL-1.2.0-RC16.18-ISSUE-NAVIGATOR';
  ...
  function locate(input,{behavior='smooth'}={}){
    ...
    // Visibility switches can reflow the row; resolve the target once more afterwards.
    window.setTimeout(()=>{
  ...
  function syncFormalDialog(){
    const dialog=document.getElementById('fp-a18-formal-dialog');if(!dialog)return;
    const cancel=dialog.querySelector('[data-a18-cancel]');if(cancel)cancel.textContent='返回并定位第一项';
    const title=dialog.querySelector('[data-a18-title]');if(title)title.textContent='导出前提醒（不阻止导出）';
    const summary=dialog.querySelector('[data-a18-summary]');if(summary){summary.innerHTML=summary.innerHTML.replace(/必须补充/g,'建议补充').replace(/阻断项/g,'提醒项');}
    dialog.querySelectorAll('.fp-a18-formal-issue b').forEach(el=>{if(/必须处理/.test(el.textContent||''))el.textContent='建议补充';});
    const go=dialog.querySelector('[data-a18-continue]');if(go){go.hidden=false;go.removeAttribute('hidden');go.textContent='仍然导出当前版本';}
  }
  ...
  function bindFormalDialog(){
    ...
    // Only the small formal dialog subtree is observed. Never observe body/#piPaper:
    // pagination mutates hundreds of nodes and RC16.14 amplified that into UI stalls.
  ...
  function startDialogDiscovery(){
    ...
    // Formal gate appends the dialog directly to body; subtree observation is unnecessary.
  ...
  document.documentElement.dataset.huidiIssueNavigator='rc16.16';
})();
