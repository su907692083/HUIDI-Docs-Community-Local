...
function openResult(view,id){if(window.HUIDIWorkspaceClosure?.focusEntity?.(view,id))return;nav(view);setTimeout(()=>{const scope=$(`#view-${view}`);if(!scope)return;const el=$$('[data-id]',scope).find(x=>String(x.dataset.id)===String(id));const target=el?.closest('tr,.data-card,.mini-product')||el;if(target){target.scrollIntoView({block:'center',behavior:'smooth'});target.classList.add('workspace-r2-flash');setTimeout(()=>target.classList.remove('workspace-r2-flash'),1200);if(target.tagName==='TR')target.click()}},80)}
