...
function boot(){document.body.classList.add('workspace-r6');document.body.dataset.workspaceRelease=VERSION;loadNotificationCenter();refresh();document.addEventListener('click',e=>{if(e.target.closest('[data-action],.nav-btn'))setTimeout(()=>{enhanceDialog();refresh()},20);const sm=e.target.closest('.workspace-r6-row-more summary');if(sm){$$('.workspace-r6-row-more[open]').forEach(x=>{if(x!==sm.parentElement)x.open=false})}});document.addEventListener('close',e=>{if(e.target?.id==='appDialog')refresh()},true);window.addEventListener('HUIDI:local-data-change',()=>setTimeout(refresh,40));setTimeout(refresh,120)}
