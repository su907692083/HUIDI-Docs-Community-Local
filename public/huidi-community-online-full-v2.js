...
async function renderSources(pane){pane.innerHTML=empty('正在读取数据来源…');const out=await api('/api/service-connections'),rows=out.items||[];pane.innerHTML=`<div class="fv2-panel"><div class="fv2-section-title"><div><h4>数据来源</h4><p>企业核验、贸易记录、关税、船期等可使用平台来源，也可给当前公司单独配置。</p></div></div><div class="fv2-list">${rows.length?rows.map(x=>`<div class="fv2-item" data-fv2-source="${esc(x.service_key)}"><div class="fv2-source"><div><b>${esc(x.name)}</b><span>${esc(x.source==='company'?(x.connected?'本公司已连接':'本公司已停用'):x.source==='server'?'平台已有可用来源':'未连接')}</span></div><div class="fv2-actions"><span class="fv2-badge ${x.connected?'ok':'warn'}">${x.connected?'可以使用':'未连接'}</span>${button('检查',`data-fv2-source-test="${esc(x.service_key)}"`,x.connected?'':'disabled')}${button('设置',`data-fv2-source-edit="${esc(x.service_key)}"`)}</div></div><div class="fv2-hidden" data-fv2-source-form="${esc(x.service_key)}" style="margin-top:8px"><div class="fv2-form"><div class="fv2-field full"><label>服务商连接地址</label><input class="fv2-input" data-source-url value="${esc(x.endpoint_url||'')}"></div><div class="fv2-field full"><label>授权信息</label><input class="fv2-input" data-source-token type="password" placeholder="${x.token_saved?'已保存，留空表示不修改':'由数据服务商提供'}"></div></div><label style="font-size:8.5px;color:#65768b"><input data-source-enabled type="checkbox" ${x.source==='company'&&!x.connected?'':'checked'}> 启用</label><div class="fv2-actions" style="margin-top:7px">${button('保存',`data-fv2-source-save="${esc(x.service_key)}"`,'primary')}${x.source==='company'?button('恢复平台来源',`data-fv2-source-reset="${esc(x.service_key)}"`):''}</div></div></div>`).join(''):empty('暂时没有可设置的数据来源。')}</div></div>`;pane.onclick=async e=>{const edit=e.target.closest('[data-fv2-source-edit]'),test=e.target.closest('[data-fv2-source-test]'),save=e.target.closest('[data-fv2-source-save]'),reset=e.target.closest('[data-fv2-source-reset]');try{if(edit){pane.querySelector(`[data-fv2-source-form="${CSS.escape(edit.dataset.fv2SourceEdit)}"]`)?.classList.toggle('fv2-hidden');return}if(test){await api(`/api/service-connections/${encodeURIComponent(test.dataset.fv2SourceTest)}/test`,{method:'POST',body:'{}'});toast('连接检查通过');return}if(save){const card=pane.querySelector(`[data-fv2-source="${CSS.escape(save.dataset.fv2SourceSave)}"]`),payload={endpoint_url:clean(card.querySelector('[data-source-url]').value),token:card.querySelector('[data-source-token]').value,enabled:card.querySelector('[data-source-enabled]').checked,clear_token:false};if(!payload.endpoint_url&&payload.enabled)throw new Error('请填写连接地址，或关闭这个数据来源');await api('/api/service-connections/'+encodeURIComponent(save.dataset.fv2SourceSave),{method:'PUT',body:JSON.stringify(payload)});toast('数据来源已保存');await renderSources(pane);return}if(reset){if(!confirm('恢复平台来源后，将删除当前公司的单独设置。继续吗？'))return;await api('/api/service-connections/'+encodeURIComponent(reset.dataset.fv2SourceReset),{method:'DELETE'});await renderSources(pane)}}catch(err){toast(err.message||String(err),'error')}}}
