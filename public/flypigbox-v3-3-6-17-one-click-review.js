...
(()=>{'use strict';
  ...
  function languageLabel(code){return ({bilingual:'中英双语',zh:'中文',en:'English',es:'Español',pt:'Português (Brasil)',de:'Deutsch',fr:'Français',it:'Italiano',ru:'Русский',ar:'العربية',ja:'日本語',ko:'한국어',tr:'Türkçe',nl:'Nederlands',pl:'Polski',vi:'Tiếng Việt',id:'Bahasa Indonesia',th:'ไทย'})[code]||code||'English';}
  ...
  function openTaskDialog(kind){
    ...
    if(kind==='document'){
      ...
      fields.innerHTML=`<label>选择客户<select name="customer_id">${options(st.customers||[],'请选择客户')}</select></label><label>关联业务<select name="deal_id">${options(st.deals||[],'暂不关联业务')}</select></label><label>单据类型<select name="document_type"><option value="quotation">报价单</option><option value="proforma_invoice">形式发票（PI）</option><option value="commercial_invoice">商业发票</option><option value="packing_list">装箱单</option><option value="sales_contract">销售合同</option></select></label><label>输出语言<select name="language"><option value="bilingual">中英双语</option><option value="zh">中文</option><option value="en">English</option><option value="es">Español</option><option value="pt">Português (Brasil)</option><option value="de">Deutsch</option><option value="fr">Français</option><option value="it">Italiano</option><option value="ru">Русский</option><option value="ar">العربية</option><option value="ja">日本語</option><option value="ko">한국어</option><option value="tr">Türkçe</option><option value="nl">Nederlands</option><option value="pl">Polski</option><option value="vi">Tiếng Việt</option><option value="id">Bahasa Indonesia</option><option value="th">ไทย</option></select></label><label>币种<input name="currency" value="${esc(defaultCurrency)}" maxlength="8" placeholder="USD"></label><label>使用品牌<select name="brand_id">${options(st.brands||[],'使用默认品牌')}</select></label><label class="full">已选商品<input value="${products.length} 个" readonly><small>${products.length?esc(products.slice(0,5).map(productTitle).join('、'))+(products.length>5?' 等':''):'可继续到准备页选择商品'}</small></label><label class="full">客户要求或单据说明<textarea name="request_text" rows="7" placeholder="粘贴客户聊天、商品数量、价格、交期、包装和付款要求">${esc(readPending())}</textarea></label><div class="fp-ai-task-warning full">系统只准备草稿上下文。缺少商品、数量、单价、币种、重量或箱规时，仍需要进入编辑器补齐并人工确认。</div>`;
    }else{
