/* HUIDI V3.3.6.6 — 云端数据状态、RLS部署识别与业务资料链接安全 */
(()=>{
  'use strict';
  ...
  const cloudCore=window.FlypigBOXCloudCore||null;
  let client=cloudCore?.getClient?.()||window.FlypigBOXSupabaseClient||null;
  try{
    if(!client&&!isLocal&&cfg.url&&cfg.publishableKey&&window.supabase?.createClient){
      client=window.supabase.createClient(String(cfg.url).replace(/\/$/,''),cfg.publishableKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true,flowType:'pkce'}});
      window.FlypigBOXSupabaseClient=client;
    }
  ...
  async function sha256(text){
    ...
    const bytes=new TextEncoder().encode(String(text));const hash=await crypto.subtle.digest('SHA-256',bytes);
  ...
  async function user(){if(!client)return null;try{await cloudCore?.ready?.();const shared=cloudCore?.getUser?.();if(shared)return shared;const {data}=await client.auth.getUser();return data?.user||null;}catch{return null;}}
  ...
  function ensureDataPanel(){
    ...
    if(!panel){
      ...
      panel.innerHTML=`<div class="fp-data-health-head"><div><b>云端数据状态与账号安全</b><span>同时识别服务端RLS部署状态；跨账号隔离仍需用两个测试账号完成验收。</span></div><button type="button" class="btn secondary" data-fp-data-check>检查云端状态</button></div><div id="fp-data-account-summary" class="fp-data-account-summary"></div><div id="fp-data-security-summary" class="fp-data-security-summary"><b>服务端安全状态待检查</b><span>部署V3.3.6.6数据库迁移后，这里会显示RLS是否已启用。</span></div><div id="fp-data-health-grid" class="fp-data-health-grid"><article><b>等待检查</b><span>打开本窗口后可检查当前账号的数据表和记录数量。</span></article></div>`;
    ...
    if(footer&&!$('[data-fp-export-all]',footer)){
      const all=document.createElement('button');all.type='button';all.className='btn primary';all.dataset.fpExportAll='1';all.textContent='导出当前账号业务数据';
  ...
  async function checkSecurityStatus(panel){
    const target=$('#fp-data-security-summary',panel);if(!target)return;
    if(isLocal||!client){target.className='fp-data-security-summary warn';target.innerHTML='<b>本地预览无法核验服务端安全</b><span>请进入线上版并登录后检查。</span>';return;}
    try{
      const {data,error}=await client.rpc('flypigbox_security_status');
      if(error){
        const info=classifyError(error);
        if(error.code==='PGRST202'||/function|schema cache|not found/i.test(clean(error.message))){target.className='fp-data-security-summary warn';target.innerHTML='<b>V3.3.6.6服务端安全迁移尚未识别</b><span>前台不会假装RLS已经部署；请由管理员审核并执行独立SQL包。</span>';return;}
        target.className='fp-data-security-summary bad';target.innerHTML=`<b>${esc(info.label)}</b><span>${esc(info.detail)}</span>`;return;
      }
      const result=Array.isArray(data)?data[0]:data;const tables=result?.tables||[];const missing=tables.filter(item=>!item.rls_enabled||Number(item.owner_policy_count||0)<1);
      if(result?.ok&&!missing.length){target.className='fp-data-security-summary ok';target.innerHTML=`<b>服务端RLS已识别 · ${tables.length}个数据表</b><span>当前账号仍需配合第二个测试账号完成跨账号读取、修改和删除隔离验收。</span>`;}
      else{target.className='fp-data-security-summary warn';target.innerHTML=`<b>服务端安全配置尚未完整</b><span>${missing.length?`${missing.length}个数据表未通过RLS状态检查。`:'请执行数据库迁移并重新检查。'}</span>`;}
    }catch(error){target.className='fp-data-security-summary bad';target.innerHTML=`<b>安全状态检查失败</b><span>${esc(error?.message||error)}</span>`;}
  }
  async function checkDataHealth(){
    const panel=ensureDataPanel();if(!panel)return;
    checkSecurityStatus(panel);
  ...
  async function exportCurrentAccountData(){
    if(isLocal||!client){statusText('当前为本地预览模式，无法导出云端数据。','error');return;}
    const current=await user();if(!current){statusText('请先登录后再导出当前账号业务数据。','error');return;}
    statusText('正在读取当前账号可访问的业务资料…');
    const payload={meta:{product:'HUIDI',format:'flypigbox-account-export',schema_version:1,app_version:'V3.3.6.6',exported_at:new Date().toISOString(),account_email:current.email||'',account_id:current.id,scope:'仅当前登录账号可访问的user_id数据',restore_support:'当前版本仅提供审计型导出，尚未提供一键恢复'},tables:{},errors:{},manifest:{row_counts:{},content_sha256:''}};
    for(const [table,label] of TABLES){
      try{
        const {data:rows,error}=await client.from(table).select('*').eq('user_id',current.id);
        if(error)payload.errors[table]={label,message:error.message||String(error),code:error.code||''};
        else{payload.tables[table]=rows||[];payload.manifest.row_counts[table]=(rows||[]).length;}
      }catch(error){payload.errors[table]={label,message:error?.message||String(error)};}
    }
    const canonical=JSON.stringify({meta:payload.meta,tables:payload.tables,errors:payload.errors,manifest:{row_counts:payload.manifest.row_counts}});
    payload.manifest.content_sha256=await sha256(canonical);
    const date=new Date().toISOString().slice(0,10);download(`HUIDI_当前账号业务数据_${date}.json`,JSON.stringify(payload,null,2));
    statusText(`账号数据已导出：${Object.keys(payload.tables).length}个数据表成功，${Object.keys(payload.errors).length}个表未导出。该文件当前用于留档和审计，不等于已具备一键恢复。`,'ok');
  }
  function exportLocalSettings(){
    const data={meta:{product:'HUIDI',version:'V3.3.6.6',exported_at:new Date().toISOString(),note:'仅包含 HUIDI 本机设置和临时数据，不包含登录令牌。'},localStorage:safeLocalKeys(localStorage),sessionStorage:safeLocalKeys(sessionStorage)};
  ...
  async function renderAttachmentList(dealId,section){
    ...
    const rows=data||[];
    list.innerHTML=rows.length?rows.map(row=>{const item=decodeAttachment(row.summary);const category=attachmentCategoryLabel(item.category||row.channel?.replace(/^业务资料·/,''));return `<article><div><b>${esc(item.title||'未命名资料')}</b><small>${esc(category)} · ${esc(fmtTime(row.created_at))}</small>${item.note?`<span>${esc(item.note)}</span>`:''}</div><div class="fp-attachment-actions">${safeHttpUrl(item.url)?`<a href="${esc(safeHttpUrl(item.url))}" target="_blank" rel="noopener noreferrer">打开链接</a>`:''}<button type="button" data-fp-delete-attachment="${esc(row.id)}" data-deal-id="${esc(dealId)}">删除索引</button></div></article>`;}).join(''):'<div class="fp-attachment-empty"><b>还没有资料索引</b><span>可以记录付款水单、验货报告、物流文件或云盘链接；这里不会上传文件内容。</span></div>';
  }
  ...
  async function saveAttachment(form){
    ...
    if(url&&!safeHttpUrl(url)){alert('只允许填写完整的 http:// 或 https:// 在线链接；javascript、data、file 等协议已被阻止。');return;}
    const payload={user_id:current.id,deal_id:dealId,activity_type:'attachment',channel:`业务资料·${attachmentCategoryLabel(category)}`,summary:encodeAttachment({category,title,url:safeHttpUrl(url),note})};
  ...
  function install(){
    ...
    document.addEventListener('click',event=>{
      ...
      if(event.target.closest('[data-fp-export-all]'))exportCurrentAccountData();
    ...
    guardAccountContext();
    if(cloudCore?.subscribe)cloudCore.subscribe(detail=>{if(!['SUBSCRIBED','SESSION_REFRESHED','ENTITLEMENTS_UPDATED'].includes(detail.event))setTimeout(guardAccountContext,0);},{immediate:false});
    else if(client)client.auth.onAuthStateChange(()=>setTimeout(guardAccountContext,0));
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install);else install();
  window.FlypigBOXDataSafety={checkDataHealth,exportCurrentAccountData,exportAllCloud:exportCurrentAccountData,exportLocalSettings};
})();
