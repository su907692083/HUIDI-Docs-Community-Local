...
def exercise(base:str,output:Path)->None:
 ...
 with sync_playwright() as pw:
  ...
  try:
   ...
   page.wait_for_function("id => {const row=document.querySelector(`#hospBatchDialog [data-hosp-batch-lead=\"${id}\"]`);return (row?.querySelector('[data-hosp-contact-state]')?.innerText||'').includes('未完成')}",arg=ids[1],timeout=10000)
   ...
   page.wait_for_function("id => {const row=document.querySelector(`#hospBatchDialog [data-hosp-batch-lead=\"${id}\"]`);return (row?.querySelector('[data-hosp-contact-state]')?.innerText||'').includes('已有邮箱')}",arg=ids[1],timeout=8000)
