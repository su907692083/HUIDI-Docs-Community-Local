...
def exercise(base:str,output:Path)->None:
 ...
 with sync_playwright() as pw:
  ...
  try:
   ...
   page.wait_for_function("() => {const t=document.querySelector('[data-hosp-seq-status]')?.innerText||'';return t.includes('已为 2 个买家准备跟进计划')||t.includes('已准备 2/2 个')}",timeout=12000)
   ...
   check('batch UI tells user to review in original auto-follow-up owner','自动跟进设置' in page.locator('#hospBatchDialog').inner_text())
   shot('03-batch-human-review-and-followup-prep')
