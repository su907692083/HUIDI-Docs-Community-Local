...
let overview=null,geojson=null,loading=null,enhanceTimer=0,selected='',scheduleQueued=false;
const REGION_VIEWS={
  '全部':'0 8 1000 475',
  '北美':'8 48 360 235',
  '拉美':'190 225 270 260',
...
function bind(){window.addEventListener('click',e=>{if(e.target.closest?.('[data-wi-open]'))scheduleBurst()},true);document.addEventListener('click',e=>{if(e.target.closest('[data-wi-region]'))setTimeout(()=>{const svg=$('.wi-country-svg');if(svg)applyRegion(svg)},30)},true);window.addEventListener('popstate',()=>{if($('#wiMap'))scheduleBurst()});scheduleBurst()}
function boot(){css();bind()}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.HUIDIWorldCountryInteraction=Object.freeze({enhance,select,refresh:scheduleBurst});
})();