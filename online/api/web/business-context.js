...
function noteBusinessDeal(input,init){const u=urlOf(input);if(!u||u.origin!==location.origin)return;const method=String(init?.method||'GET').toUpperCase();if(method!=='GET')return;const match=u.pathname.match(/^\/api\/business\/deals\/(\d+)$/);if(!match)return;if(match[1]!==dealId)dealSnapshot=null;dealId=match[1];window.HUIDIBusinessLowInputFusion?.setDealId?.(dealId)}
async function observeBusinessResponse(input,init,response){const u=urlOf(input);if(!u||u.origin!==location.origin)return response;const method=String(init?.method||'GET').toUpperCase();const dealMatch=u.pathname.match(/^\/api\/business\/deals\/(\d+)$/);if(method==='GET'&&dealMatch&&response.ok){try{const data=await response.clone().json();if(dealMatch[1]===dealId){dealSnapshot=data;window.HUIDIBusinessLowInputFusion?.acceptDeal?.(data)}}catch(_){}}
const documentMatch=u.pathname.match(/^\/api\/business\/deals\/(\d+)\/native-document$/);if(method==='POST'&&documentMatch&&response.ok){try{const data=await response.clone().json();const refId=String(data?.id||'');if(refId)await window.HUIDIBusinessLowInputFusion?.seedNativeDocument?.(documentMatch[1],refId,data?.document_type||'')}catch(_){}}
return response}
...
window.HUIDIBusinessContext=Object.freeze({leadId:()=>leadId,dealId:()=>dealId,deal:()=>dealSnapshot,clear:()=>{leadId='';dealId='';dealSnapshot=null}});
})();
