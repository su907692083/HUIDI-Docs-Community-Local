...
from app.mail_delivery import MailDeliveryLog  # noqa: E402
...
class WorkbenchActionTests(unittest.TestCase):
    ...
    def _lead(self, db, name="Buyer"):
        row = Lead(
            ...
            evidence_json="[]",
            status="replied",
            contact_email=f"sales@{name.lower()}.example",
        )
        db.add(row)
        db.flush()
        return row

    def _incoming(self, lead, now, thread="thread-1"):
        return MailboxMessage(
            mailbox_id=1,
            provider_message_id=f"in-{lead.id}-{thread}",
            thread_id=thread,
            direction="incoming",
            folder="inbox",
            ...
            try:
                ...
                db.add(
                    MailboxMessage(
                        ...
                        direction="outgoing",
                        folder="sent",
                        sender="sales@us.example",
                        recipients_json="[]",
                        subject="Re: Need quotation",
                        snippet="Sent quotation",
    ...
    def test_direct_successful_send_also_clears_reply_action(self):
        with tempfile.TemporaryDirectory() as tmp:
            token, db = self._open(tmp, 893004)
            try:
                lead = self._lead(db, "BuyerD")
                now = datetime.now(timezone.utc).replace(tzinfo=None)
    ...
    def test_deal_next_action_due_today_enters_today_tasks(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                db.add(CompanySetting(id=1, timezone_name="Asia/Tokyo", updated_by="Owner"))
                db.flush()
                customer = OnlineCustomer(company_name="Buyer B", country="JP")
                db.add(customer)
                db.flush()
                _, zone_end, zone = _day_bounds(db)
                local_due = datetime.now(zone).replace(hour=15, minute=0, second=0, microsecond=0)
                deal = OnlineDeal(
                    customer_id=customer.id,
                    title="Buyer B · hinges",
                    stage="quoting",
                    probability=40,
                    currency="USD",
                    amount=1000,
                    ...
                    next_action="发送修订报价",
                    next_action_at=local_due.strftime("%Y-%m-%dT%H:%M"),
                )
                db.add(deal)
                db.commit()
    ...
    def test_company_timezone_changes_today_utc_bounds(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                setting = db.get(CompanySetting, 1)
                setting.timezone_name = "America/New_York"
                db.commit()
