...
def _safe(value: Any, limit: int = 1000) -> str:
    text = _clean(value, max(limit * 2, 600))
...
def _list(value: Any, limit: int = 8) -> list[str]:
    if isinstance(value, list):
        ...
        rows = re.split(r"[;；\n]+", str(value or ""))
    return list(dict.fromkeys(_safe(x, 320) for x in rows if _safe(x, 320)))[:limit]
...
def project_non_price_product_facts(source: ProductBrainRecord | dict[str, Any]) -> dict[str, Any]:
    ...
    Price-like fragments embedded in free text are isolated as a second guard.
    """
    ...
    identity = {
        ...
        "name": _safe(payload.get("name"), 255),
        "sku": _clean(payload.get("sku"), 160),
        "category": _safe(payload.get("category"), 160),
        "series": _safe(payload.get("series"), 160),
    }
    technical = {
        "spec": _safe(payload.get("spec") or payload.get("specification"), 1200),
        "material": _safe(payload.get("material") or payload.get("material_grade"), 320),
        "dimensions": _safe(payload.get("dimensions") or payload.get("size"), 320),
        "unit": _clean(payload.get("unit"), 40),
        "moq": _safe(payload.get("moq"), 160),
        "lead_time": _safe(payload.get("lead_time") or payload.get("delivery_time"), 160),
    }
    compliance = {
        "certifications": _list(payload.get("certifications")),
        "hs_code": _clean(payload.get("hs_code"), 80),
        "country_of_origin": _safe(payload.get("country_of_origin") or payload.get("origin"), 120),
    }
    packaging = {
        "package_type": _safe(payload.get("package_type"), 160),
        "carton_size": _safe(payload.get("carton_size"), 160),
        "qty_per_carton": _safe(payload.get("qty_per_carton"), 120),
        "cbm": _safe(payload.get("cbm"), 120),
        "gross_weight": _safe(payload.get("gross_weight"), 120),
        "net_weight": _safe(payload.get("net_weight"), 120),
    }
    positioning = {
    ...
    return {
        ...
        "guardrails": {
            ...
            "free_text_price_fragments_isolated": True,
        },
    }
